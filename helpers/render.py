...
Original file is located at
    https://colab.research.google.com/drive/1BwKDjbdowfljNT1OrCuA33u52mvKoVGs
"""
loop = True
print("hola mundo")
import os
    ...
    while frame_idx < anim_args.max_frames:
        ...
        if prev_sample is not None:
            ...
            if loop == True:
              ...
              if n_run >= (maxx_frames - loop_sw):
                  s_run_alpha =  n_run - (maxx_frames - loop_sw) 
                  alpha = 2 / maxx_frames
                  porcent = alpha + s_run_alpha / maxx_frames * (maxx_frames / loop_sw)
                  out_porcent = 1-(porcent)
                  pre_run = n_run - (maxx_frames - loop_sw)
                  
                  print( "alpha1:", out_porcent, "alpha2:", porcent, "frame:", pre_run)
                  
                
          ##################meter aqui imagen objetivo
                  #objetivo = Image.open(Split_frames + f'{pre_run:>04}.png') 
                  #filename = f"{args.timestring}_{frame_idx-str(pre_run):05}.png"
                  filename = f"{args.timestring}_{pre_run:05}.png"
                  objetivo = os.path.join(args.outdir, filename)
                  #objetivo_array = np.array(objetivo)
                  print("archivo:", filename)
                  src1 = prev_img
                  src2 = np.array(Image.open(objetivo))#.resize(src1.shape[1::-1], Image.BILINEAR))
                  #objetivo = f"/content/drive/MyDrive/AI/StableDiffusion/2022-09/CARNE/20220916110856_00001.png"
                  
                  img1 = Image.fromarray(prev_img, 'RGB')
                  preview01 = f'/content/preview01.png'
                  img1.save(preview01)
                  
                  img2 = Image.fromarray(src2, 'RGB')
                  preview02 = f'/content/preview02.png'
                  img2.save(preview02)
                  out_porcent = 1-(porcent)
                  prev_img = src1 * (out_porcent) + src2 * porcent
                           
              else:
                  print("alpha:","frame:", n_run,"strength:", strength)    
                  prev_img = prev_img
